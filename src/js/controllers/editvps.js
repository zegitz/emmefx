(function () {
  ...
  .controller('editvpsctrl', editvpsctrl)
    editvpsctrl.$inject = ['$window','$scope', '$rootScope', '$state', '$http','$localStorage','$interval',
    '$timeout','$resource','toastr']

    function editvpsctrl($window,$scope, $rootScope, $state ,$http,$localStorage , $interval, $timeout, $resource, toastr ) {
    ...
    $scope.statusList={
	 ...
	 $ctrl.vpsID = $state.params.id;
	 //console.log($ctrl.userID);
	var addressApi = "http://netpdm.com.br:83/api";

	$scope.submitUpdateVps = function(){
    console.log($scope.vpsdata)
    var due = new Date($scope.vpsdata.due).getTime()/1000;
	 $http({
        url: addressApi+'/vps/update.php',
        method: "POST",
		headers: {
        ...
        data: {
    "id": $ctrl.vpsID, 
		"name" : $scope.vpsdata.name,
		"ip" : $scope.vpsdata.ip,
		"username" : $scope.vpsdata.username,
		"manager" : $scope.vpsdata.manager,
		"password" : $scope.vpsdata.password,
    "due" : due,
    "details" : $scope.vpsdata.details,
    "status" : $scope.vpsdata.status
		}
 ...
 $ctrl.getVpsdata = function(){
  $http({
       url: addressApi+'/vps/read_one.php',
       method: "POST",
   headers: {
   'Content-Type': "application/json; charset=UTF-8"
   },	
   ...
   .then(function(response) {
           $scope.vpsdata = response.data;
           $scope.vpsdata.due = new Date(response.data.due*1000);
     console.log( $scope.vpsdata.status);
   }, 
   function(response) { // optional
            console.log(response);
   });
